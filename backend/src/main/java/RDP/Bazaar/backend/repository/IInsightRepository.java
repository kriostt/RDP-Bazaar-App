...
public interface IInsightRepository extends JpaRepository<Product, Long> {
    ...
    @Query("SELECT p.productId, p.name, p.price, p.category, p.datePosted FROM Product p WHERE p.user.userId = :userId")
    List<Object[]> getAllProductsByUserId(Long userId);
